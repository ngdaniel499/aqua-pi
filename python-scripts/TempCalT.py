...
def open_outputfile(fpath, stationid):
    outfile = 'TempCal-' + stationid + '_' + time.strftime("%Y_%m_%d_%H")+'H.csv'
    fpathf = os.path.join(fpath,outfile)
    if os.path.exists(fpathf):
        f = open(fpathf,'a')
    else:
        f = open(fpathf,'w')
        headerline = 'Time,Tempraw,TempCal,Condraw,CondCal,SpCond,Salinity,Turbraw,TurbCal,TurbManu,ChlRaw,ChlVolts,ChlCal,CDOMRaw,CDOMVolts,CDOMCal'
...
try:
    ...
    condint = config.getfloat('Section1', 'condd')
...
#   time.sleep(1*60)
	...
	sys.stdout.flush()
        ...
        r = '%s,%s,%s,%s,%s,%s,%s' % (time.strftime("%d-%m-%Y %H:%M:%S"), TempRaw, TempCal, CondRaw, CondCal, SpCond, Salinity)
        f.write(str(r))
        f.write('\n')
    f.close
    # Stop Pump
